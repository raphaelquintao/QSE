...
var mapaDeAcentos = {};
for (var i=0; i < listaDeAcentos.length; i++){
    var letras = listaDeAcentos[i].letras.split("");
    for (var j=0; j < letras.length ; j++){
        mapaDeAcentos[letras[j]] = listaDeAcentos[i].base;
    }
}

function RemoveAccents (text_str) {
    var letras = text_str.split("");
    var clean_text = "";
    for(var i=0; i< letras.length; i++) {
        clean_text += letras[i] in mapaDeAcentos ? mapaDeAcentos[letras[i]] : letras[i];
    }
    return clean_text;
}

function ParseQuery(str_query){
    if(str_query.length > 0){
        str_query = RemoveAccents(str_query).trim();
        var temp_array = str_query.split(" ");
        var temp = "";
        $.each(temp_array, function(key, term){
            if(key == 0) temp += term;
            else temp += '%20' + term;
        });
        str_query = temp;
...
function ParseResponse(response, max_itens_show){
    //response = response.replace(/[\x00-\x1F\x7F-\x9F]/g, ""); //replace control characters
    ...
    $.each(json.result, function(){
        $.each(this, function(rank, item){
            var item = atob(item);
            result_list.append('<li>'+ rank +'- <a href="'+ item +'" target="_blank">' + item + '</a></li>'); 
        }); 
        if(max_itens_show > 0 && ++count == max_itens_show) return false;  
    });

    console.log(json);
    $("#result-message").text("About " + json.docs + " results (" + json.secs + " seconds)");
}

function GetQuery(Model, Query, max_itens_show, max_itens_get){
    LoadingStart();
    $.ajax({
        url: "search.json",
        method: "GET",
        dataType: "json",
        data: {model: Model, query: Query, trunc: max_itens_get}
    }).done(function(data){
        ParseResponse(data, max_itens_show);
        var result_msg  = $("#result-message");
        result_msg.text(result_msg.text() + " - Showing Top " + max_itens_show);
        if(max_itens_get <= 10 ) result_msg.append("<b> (Press Enter to show 200)</b>");
    }).always(function(){
        LoadingEnd();
    });
...
$( document ).ready(function(){
    ...
    function Process(itens){
        var max_itens_show = itens;
        var max_itens_get = itens;
        var model = $("input:radio[name ='models']:checked").val();
        var q_text = $("#search-box").val().toLowerCase();
         
         if(q_text = ParseQuery(q_text)){
            console.log("Query valid", q_text);
            GetQuery(model, q_text, max_itens_show, max_itens_get);
         }else {
            console.log("Query invalid", q_text);
         }
    }
    ...
    function teste(key){
        console.log( "Handler for .keypress() called.", key);
    }
    ...
    $("input:radio[name ='models']").change(function(){
        Process(200);
    });
